...
import illwillEnhanced

const
  AppName = "Hestia-NimUISetups - Illwill"
...
proc initState(showHeader: bool): UiState =
  ## showHeader: if false, hides the top app title line.
...
proc runApp(cfg: IllwillEnhanceConfig) =
  ## cfg: parsed illwill enhancer settings.
  var
    s: UiState = initState(showHeader = not cfg.disableTitlebar)
...
proc main() =
  ...
  illwillInit(fullScreen = cfg.fullScreen)
  try:
    hideCursor()
    runApp(cfg)
  finally:
